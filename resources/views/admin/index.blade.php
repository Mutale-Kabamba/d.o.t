...
<div class="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-6">
    ...
    @include('admin.nav', ['activeTab' => 'submissions'])

    <!-- Submissions Action & Export Toolbar -->
    <div class="bg-white rounded-2xl p-4 sm:p-5 border border-slate-200/90 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-sm font-black text-slate-900 uppercase tracking-wider flex items-center gap-2">
                <span class="w-2.5 h-2.5 rounded-full bg-sky-500"></span>
                Cohort Anonymous Submissions Directory
            </h2>
            <p class="text-xs text-slate-500 mt-0.5">Filter, search, review individual worksheet records, and export reports.</p>
        </div>

        <div class="flex flex-wrap items-center gap-2">
            <!-- Full Master PDF Report Export -->
            <a href="{{ route('admin.submissions.export_master_pdf') }}" class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs transition shadow-xs active:scale-95">
            ...
            <!-- CSV Export -->
            <a href="{{ route('admin.submissions.export_csv') }}" class="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs transition shadow-xs active:scale-95">
